...
<script>
// import awsEndpoint from './urlsigner'
export default {
  props: {
    id: {
      type: String,
  ...
  computed: {
    dropzoneSettings() {
      ...
      if (this.awss3 !== null) {
        ...
        this.isS3 = true
        if (this.options.autoProcessQueue !== undefined) this.wasQueueAutoProcess = this.options.autoProcessQueue
      }
  ...
  mounted() {
    ...
    let Dropzone = require('../../static/assets/js/dropzone.js') //eslint-disable-line
    Dropzone.autoDiscover = false
    this.dropzone = new Dropzone(this.$refs.dropzoneElement, this.dropzoneSettings)
    let vm = this
    vm.setOption('maxFilesize', 2)
    ...
    this.dropzone.on('addedfile', function(file) {
      // alert(file.name)
      if (file.size > vm.Max_File_Size_allowed) {
        vm.$toasted.error(vm.$t('maxFileSizeMsg'))
    ...
    this.dropzone.on('sending', function(file, xhr, formData) {
      if (vm.isS3) formData.append('s3ObjectLocation', file.s3ObjectLocation)
